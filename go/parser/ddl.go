...
func parseColumnConstraint(p *Parser) (*expr.ColumnOptionDef, error) {
	...
	if p.ParseKeyword("AUTO_INCREMENT") {
		return &expr.ColumnOptionDef{ConstraintName: constraintName, Name: "AUTO_INCREMENT"}, nil
	}
	...
	if p.ParseKeyword("GENERATED") {
		...
		if p.ParseKeywords([]string{"BY", "DEFAULT", "AS", "IDENTITY"}) {
			return &expr.ColumnOptionDef{ConstraintName: constraintName, Name: "GENERATED BY DEFAULT AS IDENTITY"}, nil
		}
